...
var dataPanel = mainWindow.add('panel', [0, 0, 600, 200], 'Text Layers:');
...
});// items: listData, Comp Used?, Actions (delete row from list)
...
/**
 * list item clicked,
 * set the viewer to the selected list item
 *
 * if debug, put selected text in textEditor field and enable it
...
list.onChange = function () {
	if (list.selection != null) {
		...
		if (debug) {
			...
			var font = fontList.indexOf(list.selection.subItems[0].text);
			if (font > -1) {
				fontDD.selection = font;
			}
		}
	}
};
...
if (debug) {
	...
	var fontDD = editPanel.add('dropdownlist', undefined, fontList);
	fontDD.selection = 0;
	 ...
	 *
	 *  delete the old listbox. create a new list box, add it to the dataPanel
	 */
