(function(thisObj) {
	...
	*/
	...
	function buildUI(thisObj) {
		...
		// Todo: Remove All Keyframes btn & function, Select a Comp dropdown? 
			...
			if (myFile) {
				if (myFile.open('r')) {
					...
					if (!templateComp || !captionLayer) {
						findTemplateComp();
						// ToDo: add the ability to select which comp to use
						// construct and select from a dropdown of existing comps? default=Template
						// rename templateComp to selectedComp?
						// alert('activeItem: '+app.project.activeItem.name);
					}

					// set factor by Template duration
					// var average = Math.round(templateComp.workAreaDuration / (txtArray.length-1));
					// var diff = factor/average;
					// factor = factor-diff;
					// alert('factor: '+factor+' - average: '+average+' - diff: '+diff+' -comp: '+templateComp.name);
